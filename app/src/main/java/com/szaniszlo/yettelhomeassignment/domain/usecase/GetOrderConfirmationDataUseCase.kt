...
class GetOrderConfirmationDataUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): OrderConfirmationData {
        val highwayInfo = highwayRepository.getHighwayInfo()
        val vehicle = highwayRepository.getVehicle()
        val counties = countyRepository.getCounties().first()
        val vignetteSelection = highwayRepository.getVignetteSelection()
        ...
        val totalCost = calculateTotalCost(countyVignettes, highwayInfo, vignetteSelection, trxFee)
